                ...
                <div class="address-form">
                  <div class="module-form-row">
                    <div class="form-item-v3">
                      <input type="text" class="js-verify" placeholder="收货人姓名" v-model.trim="receive.name">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row">
                    <div class="form-item-v3" :class="{'form-invalid-item':phoneError}">
                      <input type="text" class="js-verify" placeholder="手机号" v-model.trim="receive.phone" @focus="setPhone" @blur="inspectPhone">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  ...
                  <div class="module-form-row clear">
                    <div class="form-item-v3 select-item province-wrapper">
                      <select name="province_code" class="province select-province js-form-province js-verify" v-model="receive.provinceId">
                        ...
                        <option :value="province.area_id" v-for="province in addListData">{{province.area_name}}</option>
                      </select>
                    </div>
                  </div>
                  <div class="module-form-row clear">
                    ...
                    <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                      <select class="city select-city js-form-city js-verify" v-model="receive.countyId">
                        ...
                        <option :value="county.area_id" v-for="county in couontyList">{{county.area_name}}</option>
                      </select>
                    </div>
                  </div>
                  <div class="module-form-row">
                    <div class="form-item-v3">
                      <input type="text" class="js-verify" placeholder="详细地址，如街道名称，楼层，门牌号码等" v-model.trim="receive.add">
                      <div class="verify-error"></div>
                    </div>
                  </div>
                  <div class="module-form-row fn-clear">
                    <input type="checkbox" class="hide" value="1" v-model="receive.default">
                    <span class="blue-checkbox" :class="{'blue-checkbox-on':receive.default}" @click="checkedReceive"></span>设为默认
                  </div>
                  <!--保存按钮-->
                  <div class="dialog-blue-btn big-main-btn js-verify-address" @click="submitReceiveHandle(receive)" :class="{'disabled-btn':!isRigth}">
                    <a>保存</a>
                  </div>
                </div>
              </div>
...
<script>
  ...
  export default {
      ...
      receive : {
        handler:function(){
          if (this.receive.name === '') {
            this.isRigth = false
            return
          } else { this.isRigth = true }

          let sMobile = this.receive.phone
          ...
          if (this.receive.provinceId == 0 || this.receive.cityId == 0 || this.receive.countyId == 0) {
            this.isRigth = false
            return
          } else { this.isRigth = true }
    ...
    methods: {
      ...
      submitReceiveHandle (goods) {
        if (this.isRigth) {
          this.$emit('closePopHandle')
          this.$store.commit('submitReceiveGoods', goods)
        }
      }
    }
  }
