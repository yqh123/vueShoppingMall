<template>
	<div id="main" class="hander-car">
			...
			<div class="cart-box">
				...
				<div class="cart-inner">
					<div class="empty-label" v-if="totleCount==0">
						<h3>您的购物车中还没有商品</h3>
                        <router-link :to="{path:'/'}" class="link">现在选购</router-link>
					</div>
					<div v-else>
										...
										<div class="item-cols-num">
											<div class="select js-select-quantity">
												<span class="down" 
                                                    ...
                                                    @click="subCarPanelHandle(item.sku_id)"
                                                >-</span>
												<span class="num">{{item.count}}</span>
												<span class="up"
                                                    :class="{'up-disabled':item.count>=item.limit_num}" 
                                                    @click="plusCarPanelHandle(item.sku_id)"
                                                >+</span>
											</div>
										</div>
					...
					<div class="cart-bar-operation">
						<div>
							...
							<div class="delete-choose-goods"
                                @click="removeCheckedGoodsHandle"
                            >删除选中的商品</div>
						</div>
					</div>
					<div class="shipping">
						<div class="shipping-box">
							...
							<div class="shipping-total shipping-price">
								<h4 class="">
      								应付总额：<span>￥</span><i >{{allcheckedPrice}}</i> 
      							</h4>
								<h5 class="shipping-tips">应付总额不含运费</h5>
							</div>
						</div>
                        <span class="jianguo-blue-main-btn big-main-btn js-checkout" :class="{'disabled-btn': allcheckedNum<=0}">
                            <router-link :to="{path:changHacks}">现在结算</router-link>
                        </span>
					</div>
				</div>
			</div>
        ...
        <prompt v-if="maxOff">
            <P slot="info">商品已达到最大可购买数量，无法继续添加</P>
        </prompt>
	</div>
</template>
...
export default {
    computed: {
        ...mapState(['carPanelData', 'maxOff']),
        ...mapGetters(['totleCount', 'totlePrice', 'allChecked', 'allcheckedNum', 'allcheckedPrice']),
        changHacks () {
            let href = '/cart'
            if (this.allcheckedNum > 0) { href = '/checkout' }
            return href
        }
    },
    components: {
    ...
    methods: {
        ...mapMutations({
            delCarPanelDataHandle: 'delCarPanelData',
            plusCarPanelHandle: 'plusCarPanelData',
            subCarPanelHandle: 'subCarPanelData',
            checkGoodsHandle: 'checkGoods',
            allcheckGoodsHandle: 'allcheckGoods',
            removeCheckedGoodsHandle: 'removeCheckedGoods'
        }),
    }
}
