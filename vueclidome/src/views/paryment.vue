        ...
        <div class="content page-order-payment">
            <div class="gray-box clear">
                ...
                <div class="box-inner payment-checkout-panel clear">
                    ...
                    <span class="prices">
                        应付金额：<em><span>¥ </span>{{orderInfo.price + orderInfo.freight}}.00</em>
                    </span>
                </div>
            </div>
            ...
            <div class="confirm-detail">
                ...
                <p class="info-detail">姓名：{{orderInfo.add.name}}</p>
                <p class="info-detail">联系电话：{{orderInfo.add.phone}}</p>
                <p class="info-detail">详细地址：{{orderInfo.add.province}} {{orderInfo.add.city}} {{orderInfo.add.county}} {{orderInfo.add.add}}</p>
            </div>
            <div class="confirm-detail">
                <div class="info-title">发票信息</div>
                <p class="info-detail">发票类型：电子发票</p>
                <p class="info-detail">发票抬头：{{orderInfo.invoiceName===''? '个人' : orderInfo.invoiceName}}</p>
...
<script>
    ...
    export default {
        ...
        computed: {
            orderInfo () {
                let orderInfoData = this.$store.state.orderData.filter((goods) => {
                    return Number(goods.orderId) === Number(this.$route.query.orderId)
                })[0]
