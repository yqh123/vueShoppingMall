		...
		<div class="content page-order-checkout checkout">
						...
						<div class="address-common-table js-multiple-address-panel">
							<ul class="address-item-list clear js-address-item-list">
								<li class="js-choose-address"
                                    :class="{'selected-address-item':info.default}" 
                                    v-for="(info,index) in receiveInfo"
                                    @click="changeReceiveIndexHandle(index)"
                                >
								...
								<li class="add-address-item js-add-address" @click="tablePopShow()">
									<p>使用新地址</p>
								</li>
							</ul>
						</div>
					...
					<div class="invoice-detail"> 发票抬头：
						<div class="radio-box">
							<label> 
								...
								<span class="blue-radio" @click="tableProsonal(true)" :class="{'blue-radio-on':invoice.prosonal}"><a></a></span>  个人
							</label>
							<label>
								<input type="radio" class="hide">
								<span class="blue-radio" @click="tableProsonal(false)" :class="{'blue-radio-on':!invoice.prosonal}"><a></a></span>  单位
							</label>
						</div>
						<div class="module-form-row form-item js-invoice-title" v-if="!invoice.prosonal">
							<div class="module-form-item-wrapper no-icon small-item">
								<input type="text" placeholder="请填写公司发票抬头" class="js-verify" v-model.trim="invoice.name">
			...
			<div class="gray-box">
				...
				<div class="box-inner ui-goods-cart">
					...
					<div class="cart-table" v-for="(item,index) in checkedGoods" key="index">
						<div class="cart-group js-cart-group">
							<div class="cart-items">
								...
								<div class="name hide-row">
									<div class="name-cell">
										<a href="http://www.smartisan.com/shop/#/item/100027501" target="_blank">
				...
				<div class="box-inner">
					<div class="order-discount-line">
						<p> 商品总计： <span>¥ {{allcheckedPrice}}.00</span> </p>
				...
				<div class="box-inner">
					<div class="last-payment clear">
                        ...
                        <span class="prices fn-right">应付金额： <em>¥ {{allcheckedPrice + freight}}.00</em></span>
					</div>
				</div>
			</div>
		</div>
		<address-pop v-if="popShow" @closePopHandle="tablePopShow"></address-pop>
        <prompt v-if="maxOff">
            <P slot="info">请填写发票信息！</P>
        </prompt>
	</div>
</template>
...
<script>
    ...
    export default {
        ...
        created () {
            if (!this.$store.state.carPanelData.length) {
                this.$router.push('/')
            }
        },
        computed: {
            ...mapState(['receiveInfo', 'maxOff']),
            ...mapGetters(['totleCount', 'carPanelData', 'totlePrice', 'allChecked', 'allcheckedNum', 'allcheckedPrice', 'checkedGoods', 'filterCheckedAdd']),
            freight () {
                if (this.allcheckedPrice >= 100) {
                    return 0
                } else {
                    return 8
                }
        ...
        methods: {
            ...mapMutations({
                changeReceiveIndexHandle: 'changeReceiveIndex'
            }),
            tablePopShow () {
            ...
            submitOrderHandle () {
                if (!this.invoice.prosonal && !this.invoice.name) {
                    ...
                    let data = {
                        iDateTime: iDate.getFullYear() + '-'+ month + '-' + day,
                        orderId: iDate.getTime(),
                        goodsData: this.checkedGoods,
                        price: this.allcheckedPrice,
                        freight: this.freight,
                        invoiceName: this.invoice.name,
                        add: this.filterCheckedAdd,
                        isPay: false
                    }
